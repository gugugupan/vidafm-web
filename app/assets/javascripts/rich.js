...
function blur_start(img_id) {
    ...
    var img_width = $img_obj.attr("cwidth"), img_height = $img_obj.attr("cheight");
    var window_ratio = window_width / window_height;
    var img_ratio = img_width / img_height;
    if (window_ratio < img_ratio) {
        $img_obj.css({
    ...
    img_ele.onload = function() {
        Pixastic.process(img_ele, "blurfast", {
            amount : 2
        });
    }
...
function open_zoom() {
    ...
    zoom_interval = setInterval(function() {
        ...
        if (time_left <= 120) {
            $selector.css({
                width : initial_width + time_left * 2,
...
function open_animate() {
    ...
    switch ( animate ) {
        case 1 :
            $selector.find("#left_cover").eq(0).css({
                width : "47%",
                display : "block"
            });
            $selector.find("#right_cover").eq(0).css({
                width : "47%",
                display : "block"
            });
            $selector.find("#top_cover").eq(0).css({
                height : "100%",
                display : "block"
            });
            break;
        case 2 :
            ...
            $selector.find("#bottom_cover").eq(0).css({
                height : "47%",
                display : "block"
            });
            break;
    ...
    switch ( animate ) {
        case 1 :
            $selector.find("#top_cover").eq(0).animate({
                height : "0%"
            }, animate_speed,
            /*Call back*/
            function() {
                $selector.find("#left_cover").eq(0).animate({
                    width : "0%"
                }, animate_speed);
                $selector.find("#right_cover").eq(0).animate({
                    width : "0%"
                }, animate_speed, show_photo);
            });
            break;
        case 2 :
            $selector.find("#left_cover").eq(0).animate({
                ...
                $selector.find("#bottom_cover").eq(0).animate({
                    height : "0%"
                }, animate_speed, show_photo);
            });
            break;
        case 3 :
            $selector.find("#right_cover").animate({
                width : "90%"
            }, 0, function() {
                $selector.find("#left_cover").animate({
                    width : "90%"
                }, animate_speed);
                $selector.find("#right_cover").animate({
                    width : "0%"
