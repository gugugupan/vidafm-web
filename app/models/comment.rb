class Comment < ActiveRecord::Base
  ...
  def self.create(options = {})
  	...
  	JSON.parse(VIDA.call("comment/add", {:type => "Activity", :type_id => options[:activity_id], :content => options[:content]}, {:token => options[:user][:token], :secret => options[:user][:secret]}))["data"]
  end
end
